...
const Item = ({ item, index, isActive, toggleAccordion }) => {
  ...
  return (
    <div className={`border-b border-gray-800 ${isActive ? "bg-gray-900/30" : ""} rounded-md overflow-hidden`}>
      <button
      ...
      >
        ...
        <span className={`flex items-center justify-center w-8 h-8 rounded-full bg-white/10 transform transition-transform duration-300 ${isActive ? "rotate-45" : ""}`}>
...
const FAQ = () => {
  ...
  return (
    <section className="bg-black text-white relative overflow-hidden py-24 md:py-32" id="faq">
      {/* Éléments de fond décoratifs */}
      <div className="absolute inset-0 bg-gradient-to-br from-black via-black to-gray-900 opacity-95 z-0"></div>
      <div className="absolute top-40 right-20 w-72 h-72 rounded-full bg-purple-500/5 blur-3xl"></div>
      <div className="absolute bottom-20 left-10 w-72 h-72 rounded-full bg-blue-500/5 blur-3xl"></div>
          ...
          <div className="lg:col-span-5 lg:sticky lg:top-24">
            <div className="max-w-lg">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-6 leading-tight">
                Les réponses à vos <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-400">questions</span>
              </h2>
              <p className="text-lg text-gray-300 mb-8">
                Tout ce que vous devez savoir sur notre service de sécurité avancé et comment il peut protéger votre entreprise.
              </p>
              <a href="/contact" className="inline-flex items-center gap-2 text-white hover:text-purple-300 transition-colors">
